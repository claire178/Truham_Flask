...
# importing pymsql
import pymysql
# importing cursors from pymsql
import pymysql.cursors

# initializing the flask app
app = Flask(__name__)
# importing os fot the filepath
import os
app.config['UPLOAD_FOLDER'] = 'static/images'

# define the route
@app.route("/api/signup", methods = ["POST"])
# define the corresponding web application fuction
def signup():
    # getting user inputs
    ...
    # sql query to add the user in the database
    sql = 'insert into users(username, password, email, phone) values (%s, %s, %s, %s)'
    # defining the data to replace the pleceholders in sql query
    data = (username, password, email, phone)
    # executing the sql query
    cursor.execute(sql, data)
    # saving the data into thr database
    connection.commit()
...
def signin():
    ...
    # define the data to replace the placeholder in the sql query
    data = (email, password)
    # execute the sql query using the cursor
    cursor.execute(sql, data)
    # we are using if else statement to check how many rows are returned
    count = cursor.rowcount
    # if condtion to check if there zero rows found return no user found
    if count == 0:
        return jsonify({'Message' : "Login fail"})
    else:
        # if there isa user the in else we tell the cursor to pick only one row normally the first row
        user = cursor.fetchone()
        # return a response with the user information
        return jsonify({'Message' : 'login success', 'user' : user})
    
#add products api
# definining the route
...
# defining the correspomding web application function
